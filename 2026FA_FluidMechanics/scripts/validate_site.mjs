...
const pdfVariantRules = [
  { key: "cleaned", label: "가림막 제거 PDF 다운로드", primary: true },
...
const pages = ["index.html", "downloads.html", ...readyLectures.map(({ page }) => page)];
...
for (const page of pages) {
  ...
  if (/(private-materials|file:\/\/|(?:^|[\s"'=(>])[A-Za-z]:[\\/]|youtube\.com|youtu\.be|<\s*(?:video|audio|source)\b|\.(?:mp4|mkv|mov|webm|avi|m4v|flv|wmv|mpg|mpeg|ts|mts|m2ts|wav|m4a|mp3|aac|flac|opus|ogg)(?:["'?#\s]|$))/i.test(html)) {
...
for (const lecture of lectures) {
  for (const variant of pdfVariantRules) {
    ...
    if (typeof filename !== "string" || !/^lecture\d{2}_(?:note|original)\.pdf$/.test(filename)) {
      errors.push(`manifest: invalid ${variant.key} PDF for ${lecture.slug ?? "unknown lecture"}`);
      continue;
    }
    pdfRecords.push({ ...variant, filename, url: `${pdfDownloadBase}${filename}`, slug: lecture.slug });
  }
}
...
if (downloadPdfHrefs.length !== pdfRecords.length) {
  errors.push(`downloads.html: expected ${pdfRecords.length} PDF links, found ${downloadPdfHrefs.length}`);
}
for (const record of pdfRecords) {
  const sourcePdfPath = resolve(root, "../lecture_notes", record.filename);
  if (!existsSync(sourcePdfPath)) errors.push(`downloads.html: missing source PDF file ${record.filename}`);
  if (!hasPdfDownloadAnchor(downloads, record.url, record.filename)) {
    errors.push(`downloads.html: missing downloadable link for ${record.filename}`);
  }
}
if (!downloads.includes("가림막 제거본") || !downloads.includes("원본")) {
  errors.push("downloads.html: both cleaned and original PDF variants must be identified");
}
...
for (const lecture of lectures) {
  ...
  if (!page || !slug || !Number.isInteger(expectedSlides) || expectedSlides < 1 || typeof ready !== "boolean") {
    errors.push(`manifest: invalid lecture entry ${JSON.stringify(lecture)}`);
    continue;
  }

  const pageLink = `href="${page}"`;
  if (ready && !index.includes(pageLink)) errors.push(`index.html: missing ready lecture link to ${page}`);
  if (!ready && index.includes(pageLink)) errors.push(`index.html: pending lecture must not link to ${page}`);
  for (const variant of pdfVariantRules) {
    const filename = lecture.pdfs?.[variant.key];
  ...
  if (!lectureHtml.includes('id="concept-summary"')) errors.push(`${page}: missing standalone concept summary`);
  const sourceSections = [...lectureHtml.matchAll(/class=["'][^"']*\bsource-section\b[^"']*["']/g)].length;
  ...
  for (let index = 1; index <= expectedSlides; index += 1) {
    ...
    if (!lectureHtml.includes(`id="slide-${number}"`)) errors.push(`${page}: missing slide section ${index}`);
  }
  const hero = lectureHtml.match(/<section class="hero">([\s\S]*?)<\/section>/)?.[1] ?? "";
  const heroHrefs = attrValues(hero, "href");
  const expectedHeroHrefs = pdfVariantRules.map(({ key }) => `${pdfDownloadBase}${lecture.pdfs[key]}`);
  expectedHeroHrefs.push("index.html");
  if (heroHrefs.length !== expectedHeroHrefs.length ||
      expectedHeroHrefs.some((href) => !heroHrefs.includes(href))) {
    errors.push(`${page}: hero must contain the two PDF actions and the lecture index action`);
  }
  const heroAnchors = [...hero.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a\s*>/g)].map((match) => ({
    attrs: match[1],
  ...
  for (const variant of pdfVariantRules) {
    ...
    if (!action || !hasPdfDownloadAnchor(`<a ${action.attrs}>`, url, filename)) {
      errors.push(`${page}: invalid ${variant.key} PDF download action`);
      continue;
    }
    if (variant.primary !== action.classes.includes("button--primary")) {
      errors.push(`${page}: ${variant.key} PDF action has incorrect button emphasis`);
    }
  }
  const lectureIndexAction = heroAnchors.find(({ href }) => href === "index.html");
  if (!lectureIndexAction || lectureIndexAction.classes.includes("button--primary")) {
    errors.push(`${page}: lecture index action must keep neutral button styling`);
  }
}
