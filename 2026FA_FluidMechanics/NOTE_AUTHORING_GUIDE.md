...
한 렉처의 입력은 원본 PDF, 가림막 제거 PDF, 해당 영어 강의 전체와 타임스탬프 포함 스크립트입니다. 한 편이면 그 영상의 시간축을 사용하고, 여러 편이면 `Part 1 · 00:10:00`처럼 편 번호와 각 영상 기준 시각을 함께 표시합니다. 편 사이의 시각을 임의로 합치지 않으며, 두 편에 걸친 슬라이드는 양쪽 구간을 모두 표시합니다.
...
메인 페이지 상단 표지의 강조 버튼은 `강의자료 PDF N개`이며 `downloads.html`로 연결합니다. N은 제공하는 PDF 파일의 총 개수입니다. 다운로드 페이지 상단 표지는 중립색 `학습 노트 홈` 버튼으로 `index.html`에 연결합니다. 두 모음 페이지의 상단 표지에는 특정 렉처의 읽기 링크를 두지 않습니다. 자료 수, 날짜와 주제 등 메타데이터는 학습자가 자료를 고르는 데 필요한 내용으로 작성합니다.
...
- 여러 편인 경우 `transcripts/lectureNN/part01/`, `part02/`처럼 나누고 각 편의 시각과 검수 이력을 보존합니다.
